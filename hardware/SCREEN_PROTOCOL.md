...
| `lf` | int 0-100 | 剩余存活比例%＝消失倒计时。本条超过 `recentSec`（默认 300s）未活动就会被 PC 过滤、从列表消失；`lf=(recentSec-age)/recentSec×100`：满=刚活动过，空=即将消失。**所有会话都有**（PC 恒算出）。固件画在 `cx` 条右侧、**同一行**（不占额外垂直空间），与 `cx` 条同为点阵半透明填充，二者靠位置区分（`cx` 在左、条右带百分比数字；`lf` 在右、无数字） |
